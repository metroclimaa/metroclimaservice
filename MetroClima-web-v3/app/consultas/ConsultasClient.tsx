...
export function ConsultasClient() {
  ...
  useEffect(() => {
    ...
    supabase
      ...
      .then(({ data }) => {
        if (data?.length) setQuestions(data as PublicQuestion[]);
      });
  }, []);
  ...
  async function submitQuestion(event: FormEvent<HTMLFormElement>) {
    ...
    const payload = {
      ...
      nombre_publico: String(form.get("nombre_publico") || "Consulta anónima"),
        ...
        <div className="shell new-question-grid">
          ...
          <form className="question-form" onSubmit={submitQuestion}>
            ...
            <div className={`form-status ${status}`} aria-live="polite">
              {status === "success" && (hasSupabaseConfig ? "¡Consulta enviada! La vamos a revisar desde el panel." : "¡Consulta preparada! Esta vista está en modo demostración; quedará guardada cuando conectemos Supabase.")}
              {status === "error" && "No pudimos guardar la consulta. Probá nuevamente en unos minutos."}
            </div>
          </form>
        </div>
      </section>
