...
function Budgets({ budgets, clients, userId, onRefresh }: { budgets: Budget[]; clients: Client[]; userId: string; onRefresh: (message?: string) => Promise<void> }) {
  ...
  if (!builder) return <>
    ...
    <section className="admin-card recent-budgets full-list">
      ...
      {budgets.length ? <div className="admin-table budgets-table">
      ...
      </div> : <EmptyState title="Todavía no hay presupuestos" text="Creá el primero y quedará conectado con su futuro trabajo y reseña." />}
    </section>
  </>;
